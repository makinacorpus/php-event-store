...
abstract class AbstractEventStore implements EventStore, LoggerAwareInterface, NameMapAware
{
        ...
        $execute = function (DefaultEventBuilder $builder) {
            ...
            $callback = \Closure::bind(
                static function (Event $event) use ($aggregateId, $aggregateType, $eventName, $properties, $validAt): Event {
                    ...
                    $event->validAt = $validAt ?? $event->createdAt;
                    return $event;
                },
                null, Event::class
            );
    ...
    final protected function hydrateMessage(?string $aggregateType, ?UuidInterface $aggregateId, string $eventName, array $properties, $data)
    {
        try {
            ...
            return BrokenEnvelope::wrap($data, $properties + [
                'x-event-aggregate-id' => $aggregateId ? $aggregateId->toString() : null,
                'x-event-aggregate-type' => $aggregateType,
